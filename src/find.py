...
# Input prompt
istring: str = input(
    f"{colors.YELLOW}Keywords to find{colors.END} {colors.BOLD}{colors.RED}(Case-Sensitive){colors.END}: "
)
...
print(
    f'{colors.PURPLE}Title containing the keyword{colors.END} "{colors.RED}{istring}{colors.END}":'
)
for title, link in titles:
    if bm_match(title, istring):
        colored_title = title.replace(
            istring, f"{colors.BOLD}{colors.RED}{istring}{colors.END}"
        )
